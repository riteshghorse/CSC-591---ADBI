...
def simple(g):
	...
	# YOUR CODE HERE
	edges = g.edges.rdd
	flipped_edges = g.edges.rdd.map(lambda v: [v[1], v[0]])
	...
	# YOUR CODE HERE
	edges = edges.union(flipped_edges)
	graph_frame = sqlContext.createDataFrame(edges, ['src', 'dst']).distinct()
	# remove loops
	edges = graph_frame.filter(graph_frame.src != graph_frame.dst)
	vertices = edges.selectExpr('src as id').unionAll(edges.selectExpr('dst as id')).distinct()
	simple_g = GraphFrame(vertices, edges)
	return simple_g
...
def degreedist(g):
    ...
    # YOUR CODE HERE
	# graphframe stores edges as directed. so either take inDegree or outDegree 
	degree_counts = g.inDegrees.selectExpr('id as id', 'inDegree as degree').groupBy('degree').count()
	# print counts
...
def readFile(filename, large, sqlContext=sqlContext):
	...
	# YOUR CODE HERE
	pairs = lines.map(lambda line: line.split(delim))
	...
	# graphframe vertices
	# YOUR CODE HERE
	# endpoints = pairs.flatMap(lambda pair: (pair[0], pair[1])).distinct().toDF()
	vertices = edges.selectExpr('src as id').unionAll(edges.selectExpr('dst as id')).distinct()
...
def measureScale(distrib):
	count = distrib.map(lambda row: row[1]).collect()
	scales = powerlaw.Fit(count)
	print "Scales: "
	print scales.power_law.alpha
