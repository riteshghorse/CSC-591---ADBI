...
def similarity_metric(a, b):
  ...
  for i in range(M):
    numerator += ((a.iloc[i] - mean_a)*(b.iloc[i] - mean_b))
...
def computeDelta(wt, X, Xi):
    ...
    """
    # YOUR CODE GOES HERE
    # print(Xi.iloc[0].loc['Yi'])
    # print(Xi.iloc[0].loc[Xi.columns!='Yi'])
    # numerator = [X[] for i in range(len(Xi.index))]
    #numerator
    numerator = 0
    denominator = 0
    X = X.iloc[0:len(X)-1]
    ...
    for i in range(len(Xi.index)):
      yi = Xi.iloc[i].loc['Yi']
      xi = Xi.iloc[i].loc[Xi.columns!='Yi']
      exp = np.exp(wt * similarity_metric(X, xi))
      numerator += yi * exp
      denominator += exp
