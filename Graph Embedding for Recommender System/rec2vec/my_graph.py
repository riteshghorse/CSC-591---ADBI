...
def records_to_graph():
    ...
    node_id = 0
    for user in ratings:
        nodedict[user] = Node(node_id, user)
        nodelist.append(nodedict[user])
        node_id += 1

    for movie in movies:
        nodedict[movie] = Node(node_id, movie, 'movie')
        nodelist.append(nodedict[movie])
        node_id += 1
        # create movie_rating node for every movie for each rating
        possible_ratings = ['1', '2', '3', '4', '5']
        for rating in possible_ratings:
            nodedict[movie+"_"+rating] = Node(node_id, movie+'_'+rating, 'movie-rating')
            nodelist.append(nodedict[movie+"_"+rating])
            node_id += 1

    for director in directors:
        nodedict[director] = Node(node_id, director, 'director')
        nodelist.append(nodedict[director])
        node_id += 1
    
    for genre in genres:
        nodedict[genre] = Node(node_id, genre, 'genre')
        nodelist.append(nodedict[genre])
        node_id += 1

    for actor in actors:
        nodedict[actor] = Node(node_id, actor, 'actor')
    ...
    for i in ratings:
        user = nodedict[i]
        user_rated_movies = ratings[i]
        for movie in user_rated_movies:
            movie_node = nodedict[movie+'_'+user_rated_movies[movie]]
            user.neighbors.append(movie_node)
            movie_node.neighbors.append(user)


    for movie in movies:
        movie_node = nodedict[movie]
        movie_value = movies[movie]

        actor_value = movie_value.actors
        ...
        if movie_value.director != None:
            director_value = movie_value.director
            director_node = nodedict[director_value]
            director_node.neighbors.append(movie_node)
            movie_node.neighbors.append(director_node)
        
        genre_value = movie_value.genres
        ...
        possible_ratings = ['1', '2', '3', '4', '5']
        for rating in possible_ratings:
            rating_node = nodedict[movie+'_'+rating]
