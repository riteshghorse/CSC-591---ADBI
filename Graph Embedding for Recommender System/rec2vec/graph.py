...
def records_to_graph():
    ...
    for usr_rating in ratings:
        node = Node(nodeid, usr_rating, 'user')
        nodelist.append(node)
        nodedict[usr_rating] = node
        nodeid += 1

    for actor in actors:
        node = Node(nodeid, actor, 'actor')
        nodelist.append(node)
        nodedict[actor] = node
        nodeid += 1

    for director in directors:
        node = Node(nodeid, director, 'director')
        nodelist.append(node)
        nodedict[director] = node
        nodeid += 1

    for genre in genres:
        node = Node(nodeid, genre, 'genre')
        nodelist.append(node)
        nodedict[genre] = node
        nodeid += 1

    for movie in movies:
        node = Node(nodeid, movie, 'movie')
        nodelist.append(node)
        nodedict[movie] = node
        nodeid += 1

        for rating in rating_list:
            mov_rat = movie + "_" + rating
            node = Node(nodeid, mov_rat, 'movie-rating')
            nodelist.append(node)
            nodedict[mov_rat] = node
            nodeid += 1

    for user_rating in ratings:
        movie_ratings = ratings[user_rating]
        user_node = nodedict[user_rating]
        for movie in movie_ratings:
            rating_val = movie + "_" + movie_ratings[movie]
            rating_node = nodedict[rating_val]
