...
def feature_vecs_NLP(train_pos, train_neg, test_pos, test_neg):
    ...
    # YOUR CODE HERE
    positive_texts = collections.defaultdict(int)
    negative_texts = collections.defaultdict(int)
    
    # count words without stopwords
    for sample in train_pos:
        for word in sample:
            if word not in stopwords:
                positive_texts[word] += 1
    
    for sample in train_neg:
        for word in sample:
            if word not in stopwords:
                negative_texts[word] += 1
    
    # check for 1% condition
    pos_1 = len(positive_texts) / 100
    neg_1 = len(negative_texts) / 100

    pos_temp = {key:value for (key,value) in positive_texts.items() if value >= pos_1}
    neg_temp = {key:value for (key,value) in negative_texts.items() if value >= neg_1} 
    
    # check for twice condition
    pos_vec = {key:value for (key,value) in pos_temp.items() if value >= 2*negative_texts[key] or negative_texts[key] >= 2*value}
    neg_vec = {key:value for (key,value) in neg_temp.items() if value >= 2*positive_texts[key] or positive_texts[key] >= 2*value}

    # Using the above words as features, construct binary vectors for each text in the training and test set.
    # These should be python lists containing 0 and 1 integers.
    # YOUR CODE HERE
    features = list(pos_vec.keys()) + list(neg_vec.keys())

    train_pos_vec = []
    for sample in train_pos:
        sample_feature = {}.fromkeys(features, 0)
        for word in sample:
            if word in sample_feature:
                sample_feature[word] = 1
        train_pos_vec.append(list(sample_feature.values()))
    ...
    for sample in train_neg:
        ...
        for word in sample:
            if word in sample_feature:
                sample_feature[word] = 1
        train_neg_vec.append(list(sample_feature.values()))
    ...
    for sample in test_pos:
        ...
        for word in sample:
            if word in sample_feature:
                sample_feature[word] = 1
        test_pos_vec.append(list(sample_feature.values()))
    ...
    for sample in test_neg:
        ...
        for word in sample:
            if word in sample_feature:
                sample_feature[word] = 1
        test_neg_vec.append(list(sample_feature.values()))
...
def evaluate_model(model, test_pos_vec, test_neg_vec, print_confusion=False):
    ...
    true_positive = pos_results.count('pos')
    true_negative = neg_results.count('neg')
    false_positive = neg_results.count('pos')
    false_negatives = pos_results.count('neg')

    accuracy = float(true_positive+true_negative) / (true_positive + true_negative + false_positive + false_negatives)
    
    if print_confusion:
        print("predicted:\tpos\tneg")
        print("actual:")
        print("pos\t\t%d\t%d" % (true_positive, false_negatives))
        print("neg\t\t%d\t%d" % (false_positive, true_negative))
